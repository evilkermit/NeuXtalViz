...
class VolumeSlicerView(NeuXtalVizWidget):
    ...
    def connect_bindings(self, view_model):
        ...
        self.view_model.slice_lim_bind.connect(
            f"slice_lim{NeuXtalVizWidget.id}", self.set_slice_lim
        )
        self.view_model.cut_lim_bind.connect(
            f"cut_lim{NeuXtalVizWidget.id}", self.set_cut_lim
        )
        self.view_model.redraw_data_bind.connect(
            f"redraw_data{NeuXtalVizWidget.id}", self.redraw_data
        )
        self.view_model.slice_data_bind.connect(
            f"slice_data{NeuXtalVizWidget.id}", self.slice_data
        )
        self.view_model.redraw_data_bind.connect(
            f"cut_data{NeuXtalVizWidget.id}", self.cut_data
        )
        self.view_model.add_histo_bind.connect(
            f"add_histo{NeuXtalVizWidget.id}", self.add_histo
        )
        self.view_model.add_slice_bind.connect(
            f"add_slice{NeuXtalVizWidget.id}", self.add_slice
        )
        self.view_model.add_cut_bind.connect(
            f"add_cut{NeuXtalVizWidget.id}", self.add_cut
        )
    ...
    def connect_widgets(self, view_model):
        ...
        self.slice_ready.connect(view_model.update_slice)
        self.cut_ready.connect(view_model.update_cut)
        self.slice_combo.currentTextChanged.connect(view_model.set_slice_plane)
        self.cut_combo.currentTextChanged.connect(view_model.set_cut_line)
        self.slice_thickness_line.textChanged.connect(
            partial(view_model.set_number, "slice_thickness")
        )
        self.slice_thickness_line.editingFinished.connect(view_model.update_slice)
        self.cut_thickness_line.textChanged.connect(
            partial(view_model.set_number, "cut_thickness")
        )
        self.cut_thickness_line.editingFinished.connect(view_model.update_cut)
        self.clim_combo.currentTextChanged.connect(view_model.set_clim_clip_type)
        self.cbar_combo.currentTextChanged.connect(view_model.set_cbar)
        ...
        self.vlim_combo.currentTextChanged.connect(view_model.set_vlim_clip_type)
        self.slice_scale_combo.currentTextChanged.connect(view_model.set_slice_scale)
        self.cut_scale_combo.currentTextChanged.connect(view_model.set_cut_scale)
        self.slice_line.textChanged.connect(
            partial(view_model.set_number, "slice_value")
        )
        self.slice_line.editingFinished.connect(self.redraw_data)
        self.cut_line.textChanged.connect(partial(view_model.set_number, "cut_value"))
        self.cut_line.editingFinished.connect(view_model.update_cut)
        self.vmin_line.textChanged.connect(partial(view_model.set_number, "vmin"))
        self.vmin_line.editingFinished.connect(view_model.update_cvals)
        self.vmax_line.textChanged.connect(partial(view_model.set_number, "vmax"))
        self.vmax_line.editingFinished.connect(view_model.update_cvals)
        self.xmin_line.textChanged.connect(partial(view_model.set_number, "xmin"))
        self.xmin_line.editingFinished.connect(view_model.update_limits)
        self.xmax_line.textChanged.connect(partial(view_model.set_number, "xmax"))
        self.xmax_line.editingFinished.connect(view_model.update_limits)
        self.ymin_line.textChanged.connect(partial(view_model.set_number, "ymin"))
        self.ymin_line.editingFinished.connect(view_model.update_limits)
        self.ymax_line.textChanged.connect(partial(view_model.set_number, "ymax"))
        self.ymax_line.editingFinished.connect(view_model.update_limits)
        self.vol_scale_combo.currentTextChanged.connect(view_model.set_vol_scale)
        self.opacity_combo.currentTextChanged.connect(view_model.set_opacity)
        self.range_combo.currentTextChanged.connect(view_model.set_opacity_range)
